...
const Env = z.object({
  ...
  CORDON_SESSION_SECRET: z
    .string()
    .min(1)
    .default(() => {
      console.warn(
        "[CORDON] CORDON_SESSION_SECRET not set — using ephemeral key (not restart-safe)",
      );
...
export const oidcEnabled = !!(
  config.CORDON_OIDC_ISSUER &&
  config.CORDON_OIDC_CLIENT_ID &&
  config.CORDON_OIDC_CLIENT_SECRET
);
